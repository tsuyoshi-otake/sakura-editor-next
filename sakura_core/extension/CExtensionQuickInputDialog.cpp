...
constexpr wchar_t kWindowClass[] = L"SakuraExtensionQuickInput";
...
bool EnsureWindowClass(HINSTANCE instance) noexcept
{
	...
	windowClass.lpfnWndProc = CExtensionQuickInputDialog::WindowProcedure;
...
CExtensionQuickInputDialog::CExtensionQuickInputDialog(const SExtensionQuickInputRequest& request)
	: m_request(request)
{
	m_completion.id = request.id;
	m_completion.state = EExtensionQuickInputState::Cancelled;
}

SExtensionQuickInputCompletion CExtensionQuickInputDialog::DoModal(HWND parent) noexcept
{
	if (!parent || !::IsWindow(parent)) {
		m_completion.state = EExtensionQuickInputState::HostLost;
		return m_completion;
	}
	if (!Create(parent)) {
		m_completion.state = EExtensionQuickInputState::Overloaded;
		return m_completion;
	}
...
bool CExtensionQuickInputDialog::Create(HWND parent) noexcept
{
	m_parent = parent;
	...
	const int height = Scale(parent, m_request.kind == EExtensionQuickInputKind::QuickPick ? 410 : 190);
...
LRESULT CALLBACK CExtensionQuickInputDialog::WindowProcedure(
	HWND window, UINT message, WPARAM wParam, LPARAM lParam) noexcept
{
	auto* self = reinterpret_cast<CExtensionQuickInputDialog*>(::GetWindowLongPtrW(window, GWLP_USERDATA));
	if (message == WM_NCCREATE) {
		const auto* create = reinterpret_cast<const CREATESTRUCTW*>(lParam);
		self = static_cast<CExtensionQuickInputDialog*>(create->lpCreateParams);
...
LRESULT CExtensionQuickInputDialog::HandleMessage(UINT message, WPARAM wParam, LPARAM lParam) noexcept
{
	switch (message) {
	case WM_CREATE: {
		const auto instance = reinterpret_cast<HINSTANCE>(::GetWindowLongPtrW(m_window, GWLP_HINSTANCE));
		m_prompt = ::CreateWindowExW(0, L"STATIC",
			m_request.placeholder.empty() ? L"" : m_request.placeholder.c_str(),
			WS_CHILD | WS_VISIBLE, 0, 0, 0, 0, m_window, nullptr, instance, nullptr);
		if (m_request.kind == EExtensionQuickInputKind::QuickPick) {
...
void CExtensionQuickInputDialog::Layout(int width, int height) noexcept
{
	if (!m_window) return;
...
void CExtensionQuickInputDialog::Accept() noexcept
{
		...
		if (m_request.canPickMany) {
			...
			if (count > 0) {
				...
				const auto copied = static_cast<int>(::SendMessageW(
					m_input, LB_GETSELITEMS, static_cast<WPARAM>(selected.size()), reinterpret_cast<LPARAM>(selected.data())));
	...
	m_completion.state = EExtensionQuickInputState::Accepted;
	::DestroyWindow(m_window);
}

void CExtensionQuickInputDialog::Cancel() noexcept
{
	m_completion.state = EExtensionQuickInputState::Cancelled;
