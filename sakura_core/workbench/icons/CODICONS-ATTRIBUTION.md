...
The Activity Bar's Explorer and Source Control icon geometry is adapted from
Microsoft's `vscode-codicons` repository, pinned to commit
...
The imported SVG path data is preserved exactly by the GDI commands in
`CodiconsActivityIcons.h`. Each SVG coordinate is multiplied by 2000 into a
48,000 by 48,000 logical GDI square; an advanced-mode GDI world transform maps
that square to the existing `IconRect`. This is an implementation-only scaling
adaptation; it does not alter the glyph geometry. Fill colour continues to come
from Sakura's existing Activity Bar palette.
...
Only the generic `files` and `source-control` icons are imported. No VS Code
product logo, Microsoft logo, or other trademark-bearing icon is included.
