...
class ReleasePromotionSourceCheckTests(unittest.TestCase):
    ...
    def test_timestamp_grammar_is_ascii_bounded_and_selftested(self) -> None:
        ...
        self.assertIn("$allowedNonRequiredGitHubSkippedTimestampSkew", self.text)
        self.assertIn("-not $isRequiredCheck", self.text)
        self.assertIn("$appId -eq $TrustedAppId", self.text)
        self.assertIn("$appSlug -ceq 'github-actions'", self.text)
        self.assertIn("$isCanonicalActionsJob", self.text)
        self.assertIn("$reverseTimestampSkew -le [TimeSpan]::FromSeconds(9)", self.text)
        self.assertIn("$observedSkippedTimestampSkew", self.text)
        self.assertIn("'2026-08-30T00:10:09Z'", self.text)
        self.assertIn("$tenSecondSkippedTimestampSkew", self.text)
        self.assertIn("'2026-08-30T00:10:10Z'", self.text)
        self.assertIn("$requiredSkippedTimestampSkew", self.text)
        self.assertIn("$successfulUnrelatedTimestampSkew", self.text)
        self.assertIn("$untrustedSkippedTimestampSkew", self.text)
        self.assertIn("$legacySkippedTimestampSkew", self.text)
