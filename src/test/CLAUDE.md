...
- `cmake/` defines `tests1`, GoogleTest, Miniz, test resource ZIPs, and related dependencies.
- `msbuild/` connects those CMake targets to `tests1.vcxproj`.
...
- GoogleTest configuration and library builds must remain conditional on their declared inputs/outputs; do not rebuild GoogleTest on every `tests1` compile.
