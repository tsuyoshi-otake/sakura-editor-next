...
| F21 | REPRODUCED (部分修正) | cfddbcbb2 (部分) | io/CFileLoad.h, .cpp / constructor, Prepare | temporary設定をpointerで保持。Prepareは親のpointerへ上書き。設定snapshotを所有しPrepareへ共有、converterも共有所有。設定変更のred→greenあり。mappingは依然親の借用でF21全体は未完了。 |
