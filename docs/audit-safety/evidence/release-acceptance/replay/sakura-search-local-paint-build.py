...
def run(cmd,label):
    ...
    (out/'search-local-paint-receipt.json').write_text(json.dumps(record,indent=2),encoding='utf-8')
...
try:
    ...
    build('search-local-paint-build')
    run(['pwsh','-NoProfile','-File','C:/Users/developer/tmp/sakura-search-local-paint.ps1'],'search-local-paint')
finally:
    p.write_bytes(original)
    record['restored_sha256']=hashlib.sha256(p.read_bytes()).hexdigest()
    build('search-local-paint-normal-build')
