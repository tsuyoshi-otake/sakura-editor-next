...
- `build-on-msys2.yml` deliberately keeps **no** bootstrap retry. It needs the `.sh` entry point under its `msys2 {0}` default shell, so it cannot call the composite action, and the job is `continue-on-error: true`, so a transient bootstrap failure there turns nothing red and ends the job sooner rather than later. That is the whole reason now. **The finding debt that used to block edits to the hosted workflows is paid (#164):** their legacy hand-copied headless filters are gone. The list now lives once in `src/test/headless-suite-selection.env`; `build-on-msys2.yml` reads it into `$GITHUB_ENV` from `pwsh`, while `build-sakura.yml` reads the same file for its default and restoration paths. Never publish it from the MinGW job's default `msys2` shell, which would have to append to a Windows path. `src/test/py/test_headless_suite_selection.py` pins the arrangement.
