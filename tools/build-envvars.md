...
|SAKURA_UTF16_BACKEND|MSVC:`rust` / MinGW:`cpp`|UTF-16走査の明示バックエンド。MSVCの通常・テスト・配布ビルドは`rust`を必須とし、Cargoと固定Rust toolchainを使用する。`cpp`は実験的MinGW互換経路だけで受け付け、MSVCのコンパイル対象と製品ディスパッチから除外する。`both`、`auto`、未知の値はMSBuild/CMakeが拒否する。|
|SAKURA_UTF16_PRODUCTION_PACKAGE|`false`|配布ビルド、インストーラ、ZIPが`true`に設定する契約フラグ。配布時は`SAKURA_UTF16_BACKEND=rust`を必須とし、C++や削除済み`both`モードを指定したパッケージ処理をCargo・コンパイル・パッケージ処理の前に拒否する。|
