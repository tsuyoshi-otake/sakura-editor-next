...
|SAKURA_OUTPUT_BACKEND|MSVC:`cpp` / MinGW:`cpp`|Output authorityの明示バックエンド。UTF-16/SIMD dispatchとは独立し、`rust`はMSVCの比較・移行確認ビルドでだけ明示選択する。値はexact lowercaseの`cpp`または`rust`だけを受け付け、大文字、空白、`both`、`auto`、未知の値、MinGWの`rust`を拒否する。MSBuildでは未設定だけが`cpp`既定となり、定義済み空の環境変数と明示的な`/p:SAKURA_OUTPUT_BACKEND=`は拒否する。CMakeでは明示`-D`が環境変数より優先され、既存cacheが再configure時の環境変数変更より優先される。C1dでは既定と配布を`cpp`に保つ。|
...
|SAKURA_OUTPUT_PRODUCTION_PACKAGE|`false`|配布ビルド、インストーラ、ZIPが`true`に設定するOutput専用の契約フラグ。C1dでは`SAKURA_OUTPUT_BACKEND=cpp`だけを受け付け、UTF-16のproductionフラグから推測しない。Rust Output本番採用時は独立した採用変更でこのhard gateを更新する。MSBuild/canonical CLIではexact lowercaseの`true`または`false`を指定し、未設定だけが`false`既定となる（定義済み空の環境変数と明示的な`/p:SAKURA_OUTPUT_PRODUCTION_PACKAGE=`は拒否）。CMake directでは既存互換の`ON`/`OFF`/`TRUE`/`FALSE`（および`1`/`0`/`YES`/`NO`）を明示的に受け付ける。CMakeは既存cacheが再configure時の環境変数変更より優先され、切り替えには明示的な`-D`が必要。それ以外は暗黙変換せず拒否する。|
