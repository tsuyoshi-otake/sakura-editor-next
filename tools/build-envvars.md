...
|SAKURA_BUILD_JOBS|論理CPU数|既存 `build-*.bat` 互換shimがcanonical CLIへ渡す全体並列予算。正の整数を指定する。canonical CLIを直接使う場合は `--jobs` を優先する。この値は MSBuild へ `/m:<値>` と `/p:CL_MPCount=<値>` の両方としてそのまま渡る。論理CPU数より小さくするとコンパイル並列度がその値まで下がる。配布ビルドの assembly listing pass は `/m:1` 固定だが、`/p:CL_MPCount` にはこの値がそのまま渡る。|
...
|SAKURA_GENERATE_ASSEMBLY_LISTINGS|未設定（無効）|`1` または `true` の場合、MSVC の `/FAsu` を有効にして `.asm` 一覧を生成する。Release 製品 listing pass は `cl.exe` の中間 `.asm` を Link 直前に削除し、LTCG が同じ出力先へ最終 listing を書く。`tests1` は製品 `/GL` object を archive 経由で再リンクし、object に記録された `/Fa` 出力先を LTCG で再利用する。そのため `build-sln.bat` / `build-all.bat` は solution と tests1 をこの値を明示的に `false` にした第一段階で完成させ、製品 `sakura.vcxproj` だけを `true`・`/m:1` にした第二段階で再ビルドする。第二段階の `/m:1` はプロジェクト単位の隔離だけを担い、コンパイルは `/p:CL_MPCount=SAKURA_BUILD_JOBS` で並列に走る。製品 listing branch はリンカーに `/CGTHREADS:1` を渡し、最終 listing を書く LTCG 段を直列化する（Issue #203）。値を切り替えた次のビルドでは再コンパイルが発生する場合がある。`build-all.bat` は配布成果物のためパッケージング段階では常に `1` を保持する。CI (`build-sakura.yml`) の Release ビルドは `pull_request` 以外のトリガー（`push`/`workflow_dispatch`）でのみ `1` を設定し、`pull_request` では `0` のままにする。`zipArtifacts.bat` は値が `1`/`true` でない場合、`.asm` の収集と `Asm` zip の生成を必須とせず skip する。|
...
|SAKURA_OUTPUT_BACKEND|MSVC:`rust` / MinGW:`cpp`|Output authorityの明示バックエンド。UTF-16/SIMD dispatchとは独立する。MSVCはIssue #274によりRustを既定とし、`cpp`の明示選択がビルド単位のrollbackとなる。値はexact lowercaseの`cpp`または`rust`だけを受け付け、大文字、空白、`both`、`auto`、未知の値、MinGWの`rust`を拒否する。MSBuildでは未設定だけが`rust`既定となり、定義済み空の環境変数と明示的な`/p:SAKURA_OUTPUT_BACKEND=`は拒否する。CMakeはcompiler識別後に未設定時の既定を確定し、明示`-D`が環境変数より優先され、既存cacheが再configure時の環境変数変更より優先される。|
...
|SAKURA_OUTPUT_PRODUCTION_PACKAGE|`false`|配布ビルド、インストーラ、ZIPが`true`に設定するOutput専用の契約フラグ。Issue #274によりMSVCの`cpp`と`rust`を受け付け、UTF-16のproductionフラグから推測しない。公開用のexact provenanceは別途Rust Outputを要求する。MSBuild/canonical CLIではexact lowercaseの`true`または`false`を指定し、未設定だけが`false`既定となる（定義済み空の環境変数と明示的な`/p:SAKURA_OUTPUT_PRODUCTION_PACKAGE=`は拒否）。CMake directでは既存互換の`ON`/`OFF`/`TRUE`/`FALSE`（および`1`/`0`/`YES`/`NO`）を明示的に受け付ける。CMakeは既存cacheが再configure時の環境変数変更より優先され、切り替えには明示的な`-D`が必要。それ以外は暗黙変換せず拒否する。|
...
`cpp`/`rust`/`true`/`true`へ明示します。通常のDebug/Release branchやjob-global環境には昇格させません。
同じexact値はrelease provenanceの`build_contract`へ記録され、distribution smokeがpayload実行前に
型と大小文字を含めて照合します。Issue #274の2026-09-08承認は性能gate未達を受容し、Intel実機・長時間canaryを後回しにする例外です。未検証項目を合格とは扱いません。
