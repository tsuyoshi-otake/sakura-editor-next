...
https://github.com/tsuyoshi-otake/onigmo-next

**Onigmo NEXT** is a maintained fork of
[k-takata/Onigmo](https://github.com/k-takata/Onigmo) for
[Sakura Editor NEXT](https://github.com/tsuyoshi-otake/sakura-editor-next).
The public C API and header names (`onigmo.h`, and so on) stay
Onigmo-compatible so existing consumers can keep their include paths.

Near-term goals for this fork:

- Keep TextMate / Oniguruma-family match behavior stable for editor use
- Ship a CMake static-library build (MSVC and MinGW) with the full encoding
  set, including Shift_JIS / Windows-31J (CP932) for bregonig
- Integrate as a separately cached library instead of compiling every `.c`
  into the editor binary each time

Upstream Onigmo notes follow (historical).
...
### Case 4: CMake (static library)

   This is the path Sakura Editor NEXT's vcpkg port uses.

        cmake -S . -B build -G "Visual Studio 17 2022" -A x64
        cmake --build build --config Release

   Optional UTF-8 encoding smoke (`test_enc_utf8`):

        cmake -S . -B build -G "Visual Studio 17 2022" -A x64 -DONIGMO_BUILD_TESTS=ON
        cmake --build build --config Release
        .\build\Release\test_enc_utf8.exe

   The installed CMake package is `Onigmo` (`find_package(Onigmo CONFIG)`,
   target `Onigmo::onigmo`). `ONIG_EXTERN=extern` is PUBLIC on that target
   so MSVC consumers do not treat the static symbols as `dllimport`.
